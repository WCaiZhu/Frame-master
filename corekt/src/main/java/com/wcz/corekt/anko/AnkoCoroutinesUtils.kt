...
/** 主线程执行（ViewModel） */
fun ViewModel.runOnMain(action: () -> Unit): Job =
...
/** 异步线程执行（ViewModel） */
fun ViewModel.runOnIO(actionIO: () -> Unit): Job =
    viewModelScope.launch(Dispatchers.IO) { actionIO() }
...
/** 异步线程执行挂起函数 */
fun CoroutineScope.runOnSuspendIO(actionIO: suspend () -> Unit): Job =
    launch(Dispatchers.IO) { actionIO() }

/** 异步线程执行挂起函数（ViewModel） */
fun ViewModel.runOnSuspendIO(actionIO: suspend () -> Unit): Job =
    viewModelScope.launch(Dispatchers.IO) { actionIO() }
