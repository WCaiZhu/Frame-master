...
fun Context.getScreenBrightnessModeState(): Int = Settings.System.getInt(
    contentResolver,
...
fun Context.isScreenBrightnessModeAutomatic(): Boolean =
    getScreenBrightnessModeState() == Settings.System.SCREEN_BRIGHTNESS_MODE_AUTOMATIC

/** 设置系统屏幕亮度模式[mode]（手动：Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL ； 自动：Settings.System.SCREEN_BRIGHTNESS_MODE_AUTOMATIC） */
fun Context.setScreenBrightnessMode(mode: Int): Boolean =
    Settings.System.putInt(contentResolver, Settings.System.SCREEN_BRIGHTNESS_MODE, mode)

/** 获取手动模式下的系统亮度值，范围0-255（默认值255） */
@JvmOverloads
fun Context.getScreenBrightness(def: Int = 255): Int =
    Settings.System.getInt(contentResolver, Settings.System.SCREEN_BRIGHTNESS, def)
...
/** 获取屏幕休眠时间（单位毫秒，默认30秒） */
@JvmOverloads
fun Context.getScreenDormantTime(def: Int = 30000): Int =
    Settings.System.getInt(contentResolver, Settings.System.SCREEN_OFF_TIMEOUT, def)
...
fun Context.getAirplaneModeState(def: Int = 0): Int =
    Settings.Global.getInt(contentResolver, Settings.Global.AIRPLANE_MODE_ON, def)
...
fun Context.getRingVolume(): Int =
    (getSystemService(Context.AUDIO_SERVICE) as AudioManager).getStreamVolume(
        AudioManager.STREAM_RING
    )

/** 获取铃声音量最大值（一般是7） */
fun Context.getMaxRingVolume(): Int =
