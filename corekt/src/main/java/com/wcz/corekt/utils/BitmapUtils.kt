...
import com.wcz.corekt.anko.getScreenHeight
...
object BitmapUtils {
    ...
    @JvmStatic
    @JvmOverloads
    fun base64ToByte(base64Data: String, flags: Int = Base64.NO_WRAP): ByteArray =
        Base64.decode(base64Data, flags)
    ...
    fun drawableToBitmap(
        drawable: Drawable,
        @IntRange(from = 1) widthPx: Int = drawable.intrinsicWidth,
        @IntRange(from = 1) heightPx: Int = drawable.intrinsicHeight
    ): Bitmap? {
    ...
    private fun setInSampleSize(opts: BitmapFactory.Options, widthPx: Int, heightPx: Int): Int {
        ...
        if (pictureHeight <= heightPx && pictureWidth <= widthPx) {//图片宽高均小于设置宽高则不设置缩放比例
            return 1
        }
    ...
    fun combineBitmap(
        fg: Bitmap,
        bg: Bitmap,
        @CombinekLocationType location: Int,
        marginPx: Int = 0
    ): Bitmap {
        var newMargin = 0f
        if (marginPx > 0) {
        ...
        when (location) {
            ...
            RIGHT_BOTTOM -> canvas.drawBitmap(
                fg,
                bgWidth - fgWidth - newMargin,
                bgHeight - fgHeight - newMargin,
                paint
            )
    ...
    fun createWatermarkBitmap(
        src: Bitmap,
        watermark: Bitmap,
        @WatermarkLocationType location: Int,
        @IntRange(from = 0) marginPx: Int = 0
    ): Bitmap {
        var newMargin = 0f
        if (marginPx > 0) {
            newMargin = marginPx.toFloat()
        ...
        when (location) {
            ...
            LEFT_BOTTOM -> canvas.drawBitmap(
                watermark,
                newMargin,
                height - watermarkHeight - newMargin,
                null
            )// 在src的左下角画入水印
            RIGHT_TOP -> canvas.drawBitmap(
                watermark,
                width - watermarkWidth - newMargin,
                newMargin,
                null
            )// 在src的右上角画入水印
            RIGHT_BOTTOM -> canvas.drawBitmap(
